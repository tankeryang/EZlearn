...
class Trainer(TrainerBase):
    ...
    def get_source_data(self):
        if self.args.training_type == 'online':
            ...
            for col in DTYPE_PARAMS:
                df_source_data[col] = df_source_data[col].astype(DTYPE_PARAMS[col])
    ...
    def get_useful_data(self):
        if self.__df_source_data is None:
            ...
            if self.args.training_model == 'allocate':
                df_useful_data.drop(
                    columns=[
                        'year_code', 'main_cate', 'skc_con_sale_rate',
                        'prev_year_week_code',
    ...
    def feature_engineering(self):
        ...
        # 小样本sms
        # df_useful_data_1_sms, _ = train_test_split(df_useful_data[df_useful_data['sales_qty']==1], train_size=0.2, random_state=42)
        # # print(df_useful_data_1_sms.shape)
        # del _
        # gc.collect()

        # df_useful_data_2_sms, _ = train_test_split(df_useful_data[df_useful_data['sales_qty']==2], train_size=0.5, random_state=42)
        # # print(df_useful_data_2_sms.shape)
        # del _
        # gc.collect()

        # df_useful_data.drop(
        #     index=df_useful_data[
        #         (df_useful_data['sales_qty']==1) | (df_useful_data['sales_qty']==2)
        #     ].index,
        #     inplace=True
        # )
        # print(df_useful_data.shape)

        # return pd.concat([df_useful_data_1_sms, df_useful_data_2_sms, df_useful_data]).reset_index()
    ...
    def preprocessing(self):
        ...
        # 划分
        df_train = df_data[df_data['year_week_code'] < '201815']
        df_test = df_data[df_data['year_week_code'] >= '201815']
